...
const $$Audioquran = createComponent(async ($$result, $$props, $$slots) => {
  ...
  <\/script> `], [" ", `<div class="text-left mb-4 px-4"> <h2 class="text-3xl font-bold w-full mb-2">Audio Surat Al-Qur'an</h2> <input type="text" id="searchInput" placeholder="Cari surat berdasarkan nama, tempat turun, atau jumlah ayat..." class="w-full sm:w-1/2 border border-gray-300 px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-300"> </div> <div id="suratList" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 px-4 mb-10"> `, ` </div> <script type="module" is:client>
    const input = document.getElementById('searchInput');
    const items = document.querySelectorAll('.surat-item');
  ...
  <\/script> `])), maybeRenderHead(), suratData.data.map((surat) => {
    ...
    return renderTemplate`<div${addAttribute(`surat-item block p-4 rounded-xl border border-gray-300 shadow-sm hover:shadow-md transition duration-200 ${bgClass}`, "class")}${addAttribute(`${surat.namaLatin.toLowerCase()} ${surat.tempatTurun.toLowerCase()} ${surat.jumlahAyat}`, "data-keywords")}> <div class="text-lg font-semibold text-gray-800 mb-1"> ${surat.nomor}. ${surat.namaLatin} </div> <div class="text-sm text-gray-600 mb-2"> ${surat.jumlahAyat} ayat • Turun di ${surat.tempatTurun} </div> <select class="qari-select w-full mb-2 px-2 py-1 border border-gray-300 rounded"${addAttribute(surat.nomor, "data-nomor")}> <option value="01">Abdullah Al-Juhany</option> <option value="02">Abdul-Muhsin Al-Qasim</option> <option value="03">Abdurrahman as-Sudais</option> <option value="04">Ibrahim Al-Dossari</option> <option value="05" selected>Misyari Rasyid Al-Afasi</option> </select> <audio controls class="audio-player w-full mt-2"${addAttribute(`audio-${surat.nomor}`, "id")}> <source${addAttribute(surat.audioFull["05"], "src")} type="audio/mpeg">
Browser Anda tidak mendukung pemutar audio.
</audio> </div>`;
  })) })}`;
}, "D:/my-cv/Cv-me/src/pages/audioquran.astro", void 0);

const $$file = "D:/my-cv/Cv-me/src/pages/audioquran.astro";
