...
async function getStaticPaths({ paginate }) {
  const all_posts = await getCollection("video");
  const all_tags = all_posts.flatMap((video) => {
    return video.data.tags || [];
  });
  return all_tags.flatMap((tag) => {
    const filtred_posts = all_posts.filter((video) => {
      return video.data.tags?.includes(tag);
    });
